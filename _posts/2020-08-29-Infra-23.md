...
필자도 Github Actions를 사용해 사이드 프로젝트를 진행했었는데 변경 사항이 바로 바로 반영되니까 클라이언트 개발자분들의 반응 역시 아주 뜨거웠었다.

PR이 올라갈 때 테스트 코드를 자동으로 빌드하게 만들 수 있는데 이 부분은 크게 어렵지 않으므로 만들어둔 .yaml 파일을 공유하면서 글을 마무리하도록 하겠다.
