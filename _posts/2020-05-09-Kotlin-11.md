...
직접 애노테이션을 만들어서 **정말로 value 이외의 Attribute**에는 이름을 명시하지않아도 되는건지 확인해보자.
...
이 애노테이션은 `value`와 `greeting` 두 개의 애노테이션 애트리뷰트를 갖는다.
...
value에는 "해리"를, greeting에는 "하이요"를 애노테이션 애트리뷰트에 값으로 넘겨준 것을 확인할 수 있다.
...
그 이유는 자바에서 value 애트리뷰트를 생략할 수 있는 경우는 애트리뷰트 `하나만` 요소로 받을 수 있는 경우에 한정되기 때문이다.
...
이를 통해 코틀린 역시 자바 애노테이션에서 value를 **Special**하게 취급해준다는 사실을 알 수 있다.
