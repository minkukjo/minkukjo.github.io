...
이 분이 질문한 내용의 발단은 책 441페이지에 있는 아래의 문구 때문인데 같이 한번 봐보도록 하자.
...
나는 지금부터 이 질문에 대한 답을 하면서, Kotlin의 Annotation과 Kotlin의 Annotation에 차이에 대해서도 공유하고자 한다.

## 자바 Annotation에서 value 메소드가 Special 하다는 말의 의미?

백문이 불여일타! 직접 코드를 짜서 value 메소드가 Special 하며 애트리뷰트에 이름을 명시해야한다는 말이 무슨 말인지 직접 확인을 해보자.
...
이 애노테이션은 `value`와 `greeting` 두 개의 애노테이션 엘리먼트를 갖는다.
...
value에는 "해리"를, greeting에는 "하이요"를 애노테이션 애트리뷰트로 넘겨준 것을 확인할 수 있다.

자 그러면 value를 제외한 모든 애트리뷰트에는 이름을 명시해야한다고 했으니, value를 없애보자.
...
value 애트리뷰트를 명시하지 않아도 될 것 같지만, 그렇지 않다.

그 이유는 value 애트리뷰트를 생략할 수 있는 경우는 애트리뷰트 `하나만 요소`로 받을 수 있는 경우에 한정되기 때문이다.
...
**자바**에서는 value가 Special하며 이름을 생략해도 되는 경우는 애노테이션의 Attribute를 설정할 때 하나의 애트리뷰트 (value)만 설정하는 경우에만 가능하다.

## 코틀린의 애노테이션 + 리플렉션

코틀린의 경우 조금 더 직관적이고 간편하게 애노테이션과 리플렉션을 사용할 수 있다.
...
애노테이션의 엘리먼트들은 주생성자로 선언이 가능한데, 반드시 `val`을 붙여야한다.
...
자바와는 다르게 생성자를 기반으로 하기 때문에 애노테이션에 명시적으로 애트리뷰트를 명시하지않아도 사용할 수 있다는 점이 주목할만 하다.
...
그러면 Kotlin in Action이라는 책에서 말한 **자바 애노테이션 value 이외의 애트리뷰트에는 이름을 명시해야한다**는 처음의 문제로 돌아가보자.

## 코틀린에서 자바 애노테이션을 사용하는 경우

책에서는 **코틀린에서 자바 애노테이션을 사용할 경우 value를 자바와 동일하게 Special하게 취급해준다**는 내용이 나온다.
...
value의 경우 엘리먼트 이름을 명시하지않아도 되지만, value 이외의 애트리뷰트들은 반드시 이름을 명시할 것을 컴파일러가 경고하고 있다.

## 결론

책에서 말한 **자바 애노테이션 value 이외의 애트리뷰트에는 이름을 명시해야한다**는 의미는 **코틀린**에서 **자바 애노테이션**을 사용할 때를 의미한다.

자바 애노테이션에서의 value의 이름을 생략할 수 있는 경우는 **엘리먼트가 value 하나인 경우**에만 가능하다.

자바를 잘 알고 있다는 것을 전제로 책이 만들어졌다보니 이런 부분에서 다소 오해할 수 있겠다는 생각을 하였다.

이번 계기로 필자 역시 애노테이션과 리플렉션을 사용해볼 수 있어서 무척이나 유익하고 재밌는 시간이었다.
